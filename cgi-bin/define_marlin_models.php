...
// var_dump($model_data['models']);


//Run python for each model
foreach ($model_data['models'] as $index => $model) {
    foreach ($model as $model_name => $bots) {
        for ($i=0;$i<count($bots);$i++){
        
            $data =  [$model_name=>$result];
            array_push( $model_data['model_data'], $data);
            break;
...
$build_data = json_encode($model_data['model_ids']);
$build_all_data = json_encode($model_data);
// echo $build_data;

$filename = '/home/ident/live/current_model_data.json';
file_put_contents($filename, $build_all_data);

$path_to_exe = "/home/ident/live/build_bot_docs.py " . $build_data ;
...
$path_to_exe = "mkdocs build -f /media/marlin/Elements2/html/docs/mkdocs.yml";
